...
char	*read_rest_b(int fd, char buffer[BUFFER_SIZE + 1], char *ret, int *end)
{
...
char	*get_next_line(int fd)
{
	...
	if (read(fd, buffer[fd], 0) == -1 || BUFFER_SIZE <= 0)
		return (NULL);
	if (buffer[fd][0] == '\0')
	{
	...
	if (ret == NULL)
		return (NULL);
	while (get_bn(buffer[fd]) == 0 && end && ret != NULL)
		ret = read_rest_b(fd, buffer[fd], ret, &end);
	n = get_bn(ret);
